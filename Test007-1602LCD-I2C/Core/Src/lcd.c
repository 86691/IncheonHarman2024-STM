...
int i2c_scan() // 현재 연결되어 있는 I2C device의 주소를 표시하고 변환
{
	int addr, ret;
...
void lcd_data(char ch)	//	cmd : abcd efgh
{
	char d1, d2, data[4];
	d1 = ch & 0xf0;		//	d1 : adcd 0000
...
void lcd_init() // 1602 LCD 초기화 : Disp ON, Cursor Position, etc...
{
	...
	lcd_command(0x0f); // display mode : Disp On, CCurs On, Flash On
}
